...
class ResNet:
    ...
    # * check first layer
    # * add increase_dim
    # 레지듀얼 블록은 이게 맞고, 첫번째 레이어는 앞단에서 먼가 처리를 해주는게 맞는거 같음. 그려서 생각해보자.
    ...
    # def bottleneck_block(self, name='bottleneck_block', x):
    ...
    # of filters: {16, 32, 64}
    """
    def build_net(self, x_img, training, layer_n):
        net = x_img
    ...
    def __init__(self, name='ResNet', lr=0.001, SEED=777, layer_n=3):
        with tf.variable_scope(name):
            ...
            logits = self.build_net(x_img, self.training, layer_n=layer_n)
