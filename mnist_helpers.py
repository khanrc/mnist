...
def elastic_transform_wrapper(tensor, kernel_dim=13, sigma=6, alpha=36, negated=False):
    # [N, W, H, C]
    N = tensor.shape[0]
    ret = []
    for i in range(N):
        image = np.reshape(tensor[i], tensor[i].shape[:-1]) # WHC => WH
        distorted = elastic_transform(image, kernel_dim=kernel_dim, sigma=sigma, alpha=alpha, negated=negated)
        ret.append(np.reshape(distorted, tensor[i].shape))
